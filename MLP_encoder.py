...
import joblib
# 读取特征和目标数据
X = pd.read_csv("16group_H1N1.csv")
scaler = MinMaxScaler()
...
class WeightedMSELoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        weights = torch.where(y_true > 0.4, self.alpha, self.beta)
        weighted_mse = mse * weights
        return weighted_mse.mean()

# 定义Transformer Encoder模型
class EncoderRegressor(nn.Module):
    def __init__(self, input_dim, embed_dim, num_layers=1, num_heads=2):
        ...
        self.fc = nn.Linear(embed_dim, 1)  # 回归任务输出1个连续值
        self.input_projection = nn.Linear(input_dim, embed_dim)  # 输入特征到 embedding 的投影层

    def forward(self, x):
        x = self.input_projection(x).unsqueeze(1)  # 扩展维度以适应 transformer 输入
        x = self.encoder(x).squeeze(1)  # 压缩维度
        return self.fc(x)

# 初始化模型
input_dim = X_train.shape[1]
model = EncoderRegressor(input_dim=input_dim, embed_dim=128, num_layers=2, num_heads=4)

# 定义损失函数和优化器
criterion = WeightedMSELoss(alpha=60, beta=0.1)
...
patience = 10
counter = 0
model_path = "encoder_best_model.pth"  # 保存模型路径
...
# 加载保存的最优模型
best_model = EncoderRegressor(input_dim=input_dim, embed_dim=128, num_layers=2, num_heads=4)
